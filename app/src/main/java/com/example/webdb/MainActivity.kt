...
class MainActivity : AppCompatActivity() {
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        if(requestCode == REQUEST_CODE){
            if(resultCode == Activity.RESULT_OK){
                data?.let{
                    val user = User(it.getStringExtra(AddActivity.EXTRA_CONTACT), it.getStringExtra(AddActivity.EXTRA_NAME))
                    createUser(user)
                }
            }
        }
    ...
    companion object{
        const val TAG = "package com.example.webdb"
    }
}
